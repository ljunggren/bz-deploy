#!/bin/bash
cd "$(dirname "$0")/.."
# Fetch latest hourly backup from db1fr.boozang.com (EU)
set -e

HOST="ubuntu@db1fr.boozang.com"
BACKUP_DIR="/mnt/disk/mongodb-backups"
LOCAL_DIR="./backups/db1fr.boozang.com"
...
if [ -z "$LATEST" ]; then
  echo "ERROR: No hourly backups found on db1fr.boozang.com"
  exit 1
fi
...
echo "Fetching latest hourly backup from db1fr.boozang.com"
echo "Remote: $LATEST"
